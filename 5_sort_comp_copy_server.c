...
SO_REUSEPORT,&opt, sizeof(opt)))
 ...
 address.sin_port = htons( PORT ); //host to network byte ordering - little ending / big ending
 //The htons function takes a 16-bit number in host byte order an rleturns a
 //16-bit number in network byte order used in TCP/IP networks (the AF_INET or AF_INET6 address family).
 int temp = bind(serFileDescriptor, (struct sockaddr *)&address,
sizeof(address));
 if(temp < 0)
 {
 perror("bind failed");
 ...
 if ((newSocket = accept(serFileDescriptor, (struct sockaddr
